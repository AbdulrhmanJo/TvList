  ...
  return Promise.all([
    ...
    Promise.all(res.map((r) => r.json())).then((data) =>
      Promise.all([
        ...
        Promise.all(tr.map((t) => t.json())).then((trailers) => ({
          topRated: data[0],
          popular: data[1],
          nowPlaying: data[2],
          trending: data[3],
...
export function getMoviesOfSection(section, page) {
  ...
  if (section === "trending") {
    return fetch(`${API}/${section}/movie/week?api_key=${KEY}`).then((res) =>
      res.json()
    );
  } else {
    return fetch(
...
export async function getMovieDetails(id) {
  const response = await fetch(
    `${API}/movie/${id}?api_key=${KEY}&append_to_response=credits%2Csimilar%2Cvideos%2Crecommendations`
  );
  ...
  return Promise.all([
    ...
    Promise.all(res.map((r) => r.json())).then((data) =>
      Promise.all([
        ...
        Promise.all(tra.map((t) => t.json())).then((trailers) =>
          Promise.all([
            getNetworks(213),
            getNetworks(49),
            getNetworks(2552),
...
export function getTvOfSection(section, page) {
  ...
  console.log(section);
  if (section === "trending") {
    return fetch(`${API}/trending/tv/week?api_key=${KEY}`).then((data) =>
      data.json()
    );
  } else {
    section = section === "on_tv" ? "on_the_air" : section;
    return fetch(
      `https://api.themoviedb.org/3/tv/${section}?api_key=${KEY}&page=${page}`
    ).then((res) => res.json());
  }
}
...
export function getTvOfCategory(catagory, page) {
  return fetch(
    `${API}/discover/tv?api_key=${KEY}&sort_by=popularity.desc&page=${page}&with_genres=${catagory}`
  ).then((res) => res.json());
}
...
export function searchAll(query) {
  return fetch(
    `${API}/search/multi?api_key=${KEY}&query=${query}&include_adult=false`
  ).then((res) => res.json());
}
